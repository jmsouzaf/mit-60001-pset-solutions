...
def get_word_score(word, n):
    ...
    first_component = sum([SCRABBLE_LETTER_VALUES[letter] for letter in word.lower()])
...
def update_hand(hand, word):
    ...
    for letter in hand:
        if hand[letter] > get_frequency_dict(word.lower()).get(letter, 0):
            new_hand[letter] = hand[letter] - get_frequency_dict(word.lower()).get(letter, 0)
...
def is_valid_word(word, hand, word_list):
    ...
    if "*" in word:
        possible_words, possible_hands = [word.replace("*", vowel) for vowel in VOWELS], []
        for vowel in VOWELS:
            new_hand = hand.copy()
            new_hand[vowel] = hand.get(vowel, 0) + 1
            del (new_hand["*"])
            possible_hands.append(new_hand)

        for i in range(len(possible_words)):
            possible_word = possible_words[i]
            possible_hand = possible_hands[i]
            if is_valid_word(possible_word, possible_hand, word_list):
                return True
        ...
        for letter in get_frequency_dict(word.lower()):
            if get_frequency_dict(word.lower())[letter] > hand.get(letter, 0):
                return False
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    print(f"Ran out of letters.\nTotal score for this hand: {score}")
...
def substitute_hand(hand, letter):
    ...
    for hand_letter in hand:
        if hand_letter == letter:
            ...
            new_hand[new_letter] = hand.get(new_letter, 0) + hand[hand_letter]
        else:
            new_hand[hand_letter] = hand[hand_letter]
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    substitute = input("Would you like to substitute a letter? ").lower().strip()
    if substitute == "yes":
